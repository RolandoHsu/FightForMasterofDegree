...
TotalAssetGrowthRate <- Data %>% 
  ...
  .[str_sub(`季別`, 6, 7) == "Q4"] %>% 
  .[, `:=` (`總資產成長率` = ((`資產總額` - lag(`資產總額`)) / lag(`資產總額`))*100),
    by = "公司簡稱"] %>% 
  .[, .SD, .SDcols = c("公司簡稱", "TSE新產業名", "季別", "總資產成長率")]
...
# 其他財務比率
FinancialRatioData_WithOutJoin <- Data %>% 
  .[, `:=` (`長期資金佔不動產、廠房及設備比率(%)` = ((`非流動負債`+ `股東權益總額`)/`不動產廠房及設備`)*100,
            `不動產、廠房及設備週轉率(次)` = (`營業利益`/ `不動產廠房及設備`),
            `不動產、廠房及設備比率` = (`不動產廠房及設備` / `資產總額`)*100,
            `不動產、廠房及設備對權益比率` = (`不動產廠房及設備` / `股東權益總額`)*100,
            `營運資金對總資產比率` = ((`流動資產` - `流動負債`) / `資產總額`)*100,
            `保留盈餘對總資產比率` = (`保留盈餘` / `資產總額`)*100)] %>% 
  .[, .SD, .SDcols = c("公司簡稱", "TSE新產業名", "季別", "長期資金佔不動產、廠房及設備比率(%)", 
                       "不動產、廠房及設備週轉率(次)", "不動產、廠房及設備比率",  
                       "不動產、廠房及設備對權益比率", "營運資金對總資產比率", 
                       "保留盈餘對總資產比率")]
...
# 模型需要的財務比率變數
FinancialStructureVar <- c("負債比率", "長期資金佔不動產、廠房及設備比率(%)", "不動產、廠房及設備比率",
                           "不動產、廠房及設備對權益比率", "內部保留比率")
SolvencyVar <- c("流動比率", "利息保障倍數")
OperatingAbilityVar <- c("應收帳款週轉次數", "存貨週轉率（次）", 
                         "不動產、廠房及設備週轉率(次)", "總資產週轉次數", 
                         "總資產成長率", "權益週轉率", "營運資金週轉率", "營業費用率",
                         "研究發展費用率")
...
FinancialStatementVar <- c("公司簡稱", "TSE新產業名", "季別", FinancialStructureVar, SolvencyVar, OperatingAbilityVar, ProfitabilityVar, CashFlowVar, BalanceVar)
...
Dividend_SummaryTable %>%  
  ...
  geom_label(aes(x = `Sign`, y = `Freq(%)`, label = `Freq(%)`)) +
...
NetIncome_Negative <- FS_AllFirms_Data %>% 
  ...
  .[year(`上市櫃日期`) -1 == year(年月)] %>% 
  .[, tail(.SD, 1), by = "公司簡稱"] %>% 
  .[, NegativeNetIncome := ifelse(`常續性稅後淨利` < 0 , 1, 0)]
...
GetSummaryGroupByIPOFailAndTotal(RAndDSpending, "研究發展費用率")
...
FSVariable <- c(FinancialStructureVar, SolvencyVar, OperatingAbilityVar,
                ProfitabilityVar, CashFlowVar, BalanceVar)
...
CheckNAInFS_Final <- FS_Final_BeforeDelect %>% 
  .[, lapply(.SD, is.na), .SDcols = c(names(FS_Final_BeforeDelect)[9:39])] %>% 
  .[, lapply(.SD, sum)] %>% 
  melt(., value.name = "NA_Amount") %>% 
  .[NA_Amount != 0]

# 營收成長率與股利支付率都只有一個公司缺值，研究發展費用率僅有兩間空值，因此不用刪掉
VariableToDelect <- CheckNAInFS_Final$variable[-c(4, 6, 7)] %>% as.character()

FS_Final <- FS_Final_BeforeDelect %>% 
...
CheckNAInFS_Final_Second <- FS_Final %>% 
  .[, lapply(.SD, is.na), .SDcols = c(names(FS_Final)[9:31])] %>% 
  .[, lapply(.SD, sum)] %>% 
  melt(., value.name = "NA_Amount") %>% 
  .[NA_Amount != 0]
...
FinalData_FS <- FS_Final[, c(9:31)] %>% 
  as.data.frame() %>% 
  knnImputation(.) %>% 
  cbind(FS_Final[, c(1:8)], .) %>% 
  setDT()

CheckNAInFS_Final_Third <- FinalData_FS %>% 
  .[, lapply(.SD, is.na), .SDcols = c(names(FS_Final)[9:31])] %>% 
  .[, lapply(.SD, sum)] %>% 
  melt(., value.name = "NA_Amount") %>% 
  .[NA_Amount != 0]
...
SummaryFSData_Data <- lapply(c(names(FinalData_FS)[9:29]), 
       function(x) GetSummaryGroupByIPOFailAndTotal(FinalData_FS, x)) %>% 
  rbindlist()

DividendDummy_Summary <- FinalData_FS %>% 
  .[, .SD, .SDcols = c("IPOFail", "Dividend_Dummy", "NegativeNetIncome")] %>% 
  GetTableGroupByIPOFailOrNot(., "Dividend_Dummy") %>% 
  .[, FS_Variable := "Dividend_Dummy(%)"] 
  
NegativeNI_Summary <- FinalData_FS %>% 
  .[, .SD, .SDcols = c("IPOFail", "Dividend_Dummy", "NegativeNetIncome")] %>% 
