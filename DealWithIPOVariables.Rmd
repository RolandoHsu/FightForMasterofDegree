...
IPO_Data <- read_xlsx("Data/IPO_Data.xlsx") %>% 
  ...
  setDT() %>% 
  .[`公司簡稱` %in% ListFirm$`公司簡稱`] # 因為之前是用659家公司去抓的，樣本數改變為 604後 ，要篩掉那些在ListFirms 裡被篩掉的公司。
...
Market_CR <- map_dbl(FirmDataForMR$`公司簡稱`,
                     function(x){GetMarketReturnForEachFirm(x)})
...
FirstAndIntialFinalToJoin <- FirstAndIntialFinal %>% 
  .[, .SD, .SDcols = c("公司簡稱", 
                       "OtherIPOFirstDayReturn", "OtherIPOInitialReturn")] %>% 
  .[, `:=` (OtherIPOFirstDayReturn = OtherIPOFirstDayReturn * 100,
            OtherIPOInitialReturn = OtherIPOInitialReturn * 100)]
...
#   ggplot(., aes(x = Big4Natl, y = Percent)) +
#   geom_bar(aes(x = Big4Natl, y = Percent, fill = IPOFail),
#            stat = "identity", position = "dodge") +
#   geom_label_repel(aes(x = Big4Natl, y = Percent, label = Percent)) +
#   ggtitle("以IPO失敗與否區分Big4Natl")+
#   ylab("Percent (%)")+
#   theme_classic() +
#   theme(axis.text.x = element_text(vjust = 0.5, angle = 45),
#         plot.background  = element_rect(fill = "white"),
...
* 一年內IPO的數量平均若大於前五年的數量平均50%以上，則為熱季

* 將 NumIPOSign 以數值表示，hot season = 3, median = 2, cold season = 1

* 最後整理出 **NumIPOToJoin**，用於合併入最終data中。

* 以IPO Fail or not 建立summary來看，差距並不明顯。

```{r 建立 2002 年以前的IPO公司清單 ,echo = TRUE}
...
##### 計算 2002 年以前符合研究的IPO TSE 公司 #####
FirmListInTSE_before2002 <- FirmList_Data %>% 
  full_join(., StopTrading_Data, by = "公司簡稱") %>% 
  setDT() %>% 
  .[year(`首次TSE上市日`) >= 1992 & year(`首次TSE上市日`) < 2002 ] %>% 
  .[`首次掛牌市場` == "TSE"] %>% 
  # .[!(str_sub(`公司簡稱`, 1, 4) %in% OTCToTSE)] %>% 
  .[!(`TSE新產業名` %in% c("M2800 金融業"))] %>%
  #.[-(grep("KY$", `公司簡稱`)), ] %>% # 去除 KY股
  ...
  .[, FCD := (ifelse(is.na(`全額交割起日(二)` == T), 
                          `全額交割起日(一)`, `全額交割起日(二)`)/86400) %>% 
              as.Date() %>% as.POSIXct()] %>%
  .[!(`暫停交易原因` %in% c("併入控股公司下市", "合併下市"))] %>% # 刪除併購
...
##### 計算 2002 年以前符合研究的IPO OTC 公司 #####
FirmListInOTC_before2002 <- FirmList_Data %>% 
    full_join(., StopTrading_Data, by = "公司簡稱") %>% 
    setDT() %>% 
    .[year(`首次OTC上市日`) >= 1992 & year(`首次OTC上市日`)  < 2002] %>% 
    .[`首次掛牌市場` == "OTC"] %>% 
    .[!(`TSE新產業名` %in% c("M2800 金融業"))] %>% 
    .[, Market := "OTC"] %>% 
...
##### 建立 2002~ 2005 年間 上市 公司清單 #####
# ，因為之前的資料有篩除興櫃未滿6個月的公司，這裡要先加回來。
FirmListInTSE_2002To2005 <- FirmList_Data %>% 
  full_join(., StopTrading_Data, by = "公司簡稱") %>% 
  setDT() %>% 
  .[year(`首次TSE上市日`) >= 2002 & year(`首次TSE上市日`) < 2005 ] %>% 
  .[`首次掛牌市場` != "OTC"] %>% 
  .[!(is.na(`首次OTC上市日`) == F & `首次TSE上市日` > `首次OTC上市日`)] %>% 
  .[!(`TSE新產業名` %in% c("M2800 金融業"))] %>%
  .[, Market := "TSE"] %>% 
  dplyr::rename("上市櫃日期" = "首次TSE上市日") %>% 
  data.table(., key = c("公司簡稱", "暫停交易起日")) %>% 
...
##### OTC 2002~ 2005 #####
FirmListInOTC_2002To2005 <- FirmList_Data %>% 
    full_join(., StopTrading_Data, by = "公司簡稱") %>% 
    setDT() %>% 
    .[year(`首次OTC上市日`) >= 2002 & year(`首次OTC上市日`) < 2005 ] %>% 
    #.[order(`首次OTC上市日`)]
    .[!(`公司簡稱` %in% FirmListInTSE_2002To2005$`公司簡稱`)] %>% 
    .[!(`TSE新產業名` %in% c("M2800 金融業"))] %>% 
    .[, Market := "OTC"] %>% 
    dplyr::rename("上市櫃日期" = "首次OTC上市日") %>%
    data.table(., key = c("公司簡稱", "暫停交易起日")) %>% 
...
## 將 TSE OTC 2002年以前的每季公司數量 與本研究樣本的資料合併。

FirmListInTSE_before2014 <- ListFirm %>%
  .[Market == "TSE"] %>% 
  .[, .SD, .SDcols = c("公司簡稱", "上市櫃日期", "IPOFail")] %>% 
  .[year(`上市櫃日期`) >= 2005 ] %>% 
  rbind(., FirmListInTSE_before2002, FirmListInTSE_2002To2005)

FirmListInOTC_before2014 <- ListFirm %>%
  .[Market == "OTC"] %>% 
  .[, .SD, .SDcols = c("公司簡稱", "上市櫃日期", "IPOFail")] %>% 
  .[year(`上市櫃日期`) >= 2005 ] %>% 
  rbind(., FirmListInOTC_before2002, FirmListInOTC_2002To2005)

FullYearMonth <- c(as.Date("1992-01-01"): as.Date("2014-12-31")) %>% 
  as.Date() %>% 
  format(., "%Y-%m") %>% 
  unique() %>% 
  as.data.table() %>% 
...
# TSE 中2014年以前 每月上市數量
IPOHeatDegreeData_TSE <- copy(FirmListInTSE_before2014) %>% 
  .[, YearMonth := format(as.Date(`上市櫃日期`), "%Y-%m")] %>% 
  .[order(YearMonth)] %>% 
  .[, count(`YearMonth`)] %>%
  setnames(., "x", "YearMonth") %>% 
  full_join(., FullYearMonth, by = "YearMonth") %>% 
  setDT() %>% 
  .[, freq:= nafill(freq, type = "const", fill = 0)] %>% 
  .[order(YearMonth)]

## TSE 中2014年以前 每 年 上市數量
IPONUM_TSE_EachYear <- copy(IPOHeatDegreeData_TSE) %>% 
  .[, year := str_split(YearMonth, "-", simplify = T)[, 1]] %>% 
  .[, .SD, .SDcols = c("year", "freq")] %>% 
  .[, sum(freq), by = year] %>% 
  .[, Market := "TSE"]

# OTC 中2014年以前 每月上櫃數量
IPOHeatDegreeData_OTC <- copy(FirmListInOTC_before2014) %>% 
  .[, YearMonth := format(as.Date(`上市櫃日期`), "%Y-%m")] %>% 
  .[order(YearMonth)] %>% 
  .[, count(`YearMonth`)] %>%
  setnames(., "x", "YearMonth") %>% 
...
## OTC 中2014年以前 每 年 上市數量
IPONUM_OTC_EachYear <- copy(IPOHeatDegreeData_OTC) %>% 
  .[, year := str_split(YearMonth, "-", simplify = T)[, 1]] %>% 
  .[, .SD, .SDcols = c("year", "freq")] %>% 
  .[, sum(freq), by = year] %>% 
  .[, Market := "OTC"]
...
## 繪圖看 TSE OTC 每年上市櫃數量
IPONUM_ToPlot <- rbind(IPONUM_TSE_EachYear, IPONUM_OTC_EachYear) %>% 
  setnames(., "V1", "Quantity")

IPONUM_ToPlot %>% 
  ggplot(., aes(x = year, y = Quantity)) +
  geom_bar(aes(x = year, y = Quantity, fill = Quantity), stat = "identity") +
  geom_label(aes(x = year, y = Quantity, label = Quantity)) +
  facet_grid("Market") + 
  ggtitle("每年上市櫃數量")+
  ylab("Count")+
  theme_classic() +
...
```{r 只分冷熱季, echo = TRUE, fig.width = 10, fig.asp = 0.62, warning = FALSE, message=FALSE}
## 以1992年到目前的月 IPO 數量平均作為benchmark (HistoryIPONum) 來計算發行熱季
# TSE
CalculateNumIPO_TSE <- copy(IPOHeatDegreeData_TSE) %>%
  .[, SumBefore := cumsum(.[, 2])] %>%
  .[, `:=`(OneToNow = 1:nrow(.),
           MeanIPONum = Reduce(`+`, shift(freq, 0:11)) / 12)] %>%
  .[, HistoryIPONum := SumBefore/ OneToNow] %>%
  .[, NumIPOSign := case_when(
    MeanIPONum >= HistoryIPONum ~ "Hot Season",
    MeanIPONum < HistoryIPONum ~ "Cold Season")] %>%
  .[str_split(YearMonth, "-", simplify = TRUE)[, 1] %>% as.numeric() >= 2002] %>%
  .[, YearMonth := as.character(YearMonth)] %>%
  .[, Market := "TSE"]

CalculateNumIPO_TSE$NumIPOSign %>%
  table() %>%
  as.data.table() %>%
  setnames(., ".", "IPONumSign") %>%
  kable(., caption = "冷熱季於每一月的數量分配表 (TSE)", align = "l") %>%
  kable_styling(full_width = TRUE)

# OTC
CalculateNumIPO_OTC <- copy(IPOHeatDegreeData_OTC) %>%
  .[, SumBefore := cumsum(.[, 2])] %>%
  .[, `:=`(OneToNow = 1:nrow(.),
           MeanIPONum = Reduce(`+`, shift(freq, 0:11)) / 12)] %>%
  .[, HistoryIPONum := SumBefore/ OneToNow] %>%
...
CalculateNumIPO_OTC$NumIPOSign %>%
  table() %>%
  as.data.table() %>%
  setnames(., ".", "IPONumSign") %>%
  kable(., caption = "冷熱季於每一月的數量分配表 (OTC)", align = "l") %>%
  kable_styling(full_width = TRUE)

## 計算好OTC TSE 個別季節是冷季或熱季後，依據市場與季別丟入原data中即可。

CalculateNumIPO <- rbind(CalculateNumIPO_TSE, CalculateNumIPO_OTC)

HotIssueToPlot <- copy(CalculateNumIPO) %>%
  .[, Sign := case_when(
    NumIPOSign == "Hot Season" ~ 2,
    NumIPOSign == "Cold Season" ~ 1
  )]

HotIssueToPlot %>%
  ggplot(., aes(x = YearMonth, y = freq)) +
  geom_point()+
  ggtitle("冷熱季於每一月的數量分配表") +
  #ylim(0, 3) +
  facet_grid("Market") +
  theme_classic() +
  theme(axis.text.x = element_text(vjust = 0.5, angle = 45),
...
## 將每月的冷熱季分配導入 listfirm 中
NumIPO <- copy(ListFirm) %>%
  .[, .SD, .SDcols = c("公司簡稱", "Market", "上市櫃日期", "IPOFail")] %>%
  .[, YearMonth := format(as.Date(`上市櫃日期`), "%Y-%m")] %>%
  .[CalculateNumIPO %>%
              .[, .SD, .SDcols = c("YearMonth", "NumIPOSign", "Market")],
    on = c("YearMonth", "Market"),
    nomatch = 0] %>%
  .[order(`公司簡稱`)]
...
NumIPO_Table <- NumIPO %>%
  pull(NumIPOSign) %>%
  table() %>%
  #.[, table(NumIPOSign), by = "Market"] %>%
  as.data.table() %>%
  dplyr::rename("NumIPOSign" = ".")

kable(NumIPO_Table, caption = "研究樣本之 冷熱季數量分配表", align = "l") %>%
  kable_styling(full_width = TRUE)

## 樣本內冷熱季分配表 - 以 market 做區分
NumIPO_Table_ByMarket <- copy(NumIPO) %>%
  .[, table(NumIPOSign), by = "Market"] %>%
  as.data.table() %>%
  .[, NumIPOSign := c("Cold Season", "Hot Season", "Cold Season", "Hot Season")] %>%
  .[, .SD, .SDcols = c("Market", "NumIPOSign", "V1")] %>%
  setnames(., "V1", "N")

kable(NumIPO_Table_ByMarket, caption = "研究樣本之 冷熱季數量分配表 以Market 區分", align = "l") %>%
  kable_styling(full_width = TRUE)

# 將 NumIPOSign 以數值表示，hot season = 2, cold season = 1
NumIPOToJoin <- copy(NumIPO) %>%
...
Summary_NumIPO_ToPlot <- copy(NumIPO) %>%
...
```{r 分HotColdMedian, echo = TRUE, fig.width = 10, fig.asp = 0.62, warning = FALSE, message=FALSE}
# ## 以1992年到目前的月 IPO 數量平均作為benchmark (HistoryIPONum) 來計算發行熱季
# # TSE
# CalculateNumIPO_TSE <- copy(IPOHeatDegreeData_TSE) %>%
#   .[, SumBefore := cumsum(.[, 2])] %>%
...
#   .[, HistoryIPONum := SumBefore/ OneToNow] %>%
#   .[, NumIPOSign := case_when(
#     MeanIPONum >= 1.5*HistoryIPONum ~ "Hot Season",
#     MeanIPONum <= 0.5*HistoryIPONum ~ "Cold Season",
#     TRUE ~ "Median")] %>%
#   .[str_split(YearMonth, "-", simplify = TRUE)[, 1] %>% as.numeric() >= 2002] %>%
#   .[, YearMonth := as.character(YearMonth)] %>%
#   .[, Market := "TSE"]
...
#   .[, table(NumIPOSign), by = "Market"] %>%
#   as.data.table() %>%
#   .[, NumIPOSign := c("Cold Season", "Hot Season", "Median", "Cold Season", "Median")] %>%
#   .[, .SD, .SDcols = c("Market", "NumIPOSign", "V1")] %>%
#   setnames(., "V1", "N")
# 
# kable(NumIPO_Table_ByMarket, caption = "研究樣本之 冷熱季數量分配表 以Market 區分", align = "l") %>%
...
* 新增 **HoldingDiff_Dummy** 變數，若大於0則設為1，代表Insider淨買量 > 淨賣量 
...
InsiderHolding <- InsiderHolding_Data %>% 
  ...
  .[, .SD, .SDcols = c("公司簡稱", "年月日", "增加股數(合計)", "減少股數(合計)")] %>% 
  ...
  .[, HoldingDiff_Dummy := ifelse(HoldingDiff > 0, 1, 0)]
...
PositiveOrNot_InsiderHolding_ToPlot %>%
  .[, `:=` (IPOFail = as.character(IPOFail))]%>%
  ggplot(., aes(x = IPOFail, y = Percent)) +
  geom_bar(aes(x = IPOFail, y = Percent, fill = HoldingDiffSign),
           stat = "identity", position = "dodge") +
  geom_label(aes(x = IPOFail, y = Percent, label = Percent)) +
...
# LeadPercentage <- IPO_Data %>% 
#   .[, .SD, .SDcols = c("公司簡稱", "上市日收盤價(元)", "總承銷股數(千股)")] %>% 
#   .[, `總承銷股數(千股)` := `總承銷股數(千股)`*1000] %>% 
#   left_join(., HoldingDecrease_Data, by = "公司簡稱") %>% 
#   setDT() %>% 
#   .[, HoldingDecrease := HoldingDecrease * 1000] %>% 
#   .[, `HoldingDecreaseToUnderWrittenShares(%)` := (HoldingDecrease / `總承銷股數(千股)`)*100] %>% 
#   left_join(., ListFirm %>% .[, .SD, .SDcols = c("公司簡稱", "IPOFail")], by = "公司簡稱") %>% 
...
EmployeeSubscriptionRate_Data1 <- EmployeeSubscriptionRate_Data %>% 
  dplyr::rename("公司簡稱" = "公司", "EmployeesSubscriptionRate" = "員工認購率 %") %>% 
  left_join(., ListFirm %>% 
              .[, .SD, .SDcols = c("公司簡稱", "上市櫃日期" , "IPOFail")], 
            by = "公司簡稱") %>% 
  setDT() %>% 
  ...
  .[, `EmployeesSubscriptionRate` := sum(`EmployeesSubscriptionRate`), by = "公司簡稱"] %>% 
  .[, tail(.SD, 1), by = "公司簡稱"]

# EmployeeSubscriptionRate 用以計算summary資訊
EmployeeSubscriptionRate <- ListFirm %>% 
  .[, .SD, .SDcols = c("公司簡稱", "IPOFail")] %>% 
  left_join(., EmployeeSubscriptionRate_Data1 %>% 
              .[, .SD, .SDcols = c("公司簡稱",  "EmployeesSubscriptionRate")], 
            by = "公司簡稱") %>% 
  setDT() %>% 
  .[, `EmployeesSubscriptionRate` := 
      ifelse(is.na(`EmployeesSubscriptionRate`) == T, 0, `EmployeesSubscriptionRate`)]
...
Summary_EmployeeSubscriptionRate <- GetSummaryGroupByIPOFailAndTotal(EmployeeSubscriptionRate, "EmployeesSubscriptionRate")
...
* 以 **VCBack_ToJoin** 合併入最終data中。

```{r echo = TRUE}
load("Data/InsiderHoldSituation_Data.RData")

InsiderHoldSituation <- InsiderHoldSituation_Data %>% 
  setnames(., "公司代碼", "公司簡稱") %>% 
  .[order(`公司簡稱`)] %>% 
  left_join(., 
            ListFirm %>% 
              .[, .SD, .SDcols = c("公司簡稱", "首次REG上市日", "上市櫃日期", "IPOFail")], 
            by = "公司簡稱") %>% 
  setDT() 

# .[`上市櫃日期` %m-% months(1) <= `年月日` &  `年月日` <= `上市櫃日期` %m+% months(1) ]

InsiderHoldSituation_SameYearMonth <- InsiderHoldSituation %>% 
  .[year(`年月日`) == year(`上市櫃日期`) & month(`年月日`) == month(`上市櫃日期`)]

VC_Data <- InsiderHoldSituation_SameYearMonth %>% 
  .[str_detect(`持有人全名`, "創業投資|創投") == T]

VCBack_Vector <- unique(VC_Data$`公司簡稱`)

VCBack <- ListFirm %>% 
  .[, .SD, .SDcols = c("公司簡稱", "IPOFail", "上市櫃日期")] %>% 
  .[, VCBack := ifelse(`公司簡稱` %in% VCBack_Vector, 1, 0)]

VCBack_ToJoin <- VCBack %>% 
  .[, .SD, .SDcols = c("公司簡稱", "VCBack")]

VCBack_after2008 <- VCBack %>% 
  .[year(`上市櫃日期`) > 2008]
...
VCBack$VCBack %>% 
  ...
  as.data.table() %>% 
  setnames(., ".", "是否有創投公司投資") %>% 
  kable(., caption = "VCBack", align = "l") %>% 
  kable_styling(full_width = TRUE)

Summary_VCBack_ToPlot <- VCBack %>% 
...
JoinList <- list(IPO_DoNotNeedToDealWith, UnderwriterReputation_ToJoin, 
                 ...
                 NumIPOToJoin, InsiderHolding, 
                 EmployeeSubscriptionRateToJoin, InstitutionHolding_ToJoin, 
                 IndependentBoard_Data, VCBack_ToJoin)
...
FinalData_IPO <- FinalData_IPO_WithNA[, c(3:25)] %>% 
  ...
  setDT() %>%
  .[, .SD, .SDcols = -c("HoldingDiff_Dummy")]
  # .[, `:=` (`申購積極度` = log(1/(平均中籤率/100)))] %>% 
  # .[, .SD, .SDcols = -c("平均中籤率")]

CheckNAInFinalData_IPO <- FinalData_IPO %>% 
  .[, lapply(.SD, is.na), .SDcols = c(names(FinalData_IPO)[3:24])] %>% 
  .[, lapply(.SD, sum)] %>% 
  melt(., value.name = "NA_Amount") %>% 
  .[NA_Amount != 0]

colnames(FinalData_IPO)[c(1, 4, 5, 6, 7, 8, 22, 23)] <- 
  c("Firms", "OfferPrice", "FirstDayReturn", "WiningRate_Average",
    "UnderwrittenShares_1000", "UnderwrittenAmount_1000", 
    "InstitutionHoldingRate", "IndependentDirectorsAmount")
